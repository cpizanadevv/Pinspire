...
def edit_board_by_id(board_id):
    ...
    return (
        jsonify(
            {"message": "board changes successful", "board": board_to_edit.to_dict()}
        ),
        200,
    )
...
def delete_board_by_id(board_id):
    ...
    return (
        jsonify(
            {"message": f"Successfully deleted board {board_id}", "boardId": board_id}
        ),
        200,
    )
...
def get_pin_that_belong_to_board(board_id, pin_id):
    ...
    existing_association = (
        db.session.query(board_pins).filter_by(board_id=board_id, pin_id=pin_id).first()
    )
